...
def preprocessing():
    df = pd.read_csv('../flask/ml_service/drug/data/raw data/drug200.csv')
    df_clean = data_cleansing(df)
    df_wrag = data_wrangling(df_clean)
    df_wrag.to_csv('../flask/ml_service/drug/data/processed data/drug_clean.csv', index=False)
...
def edit_word(df):
    df['Drug'] = df['Drug'].apply(str).str.replace('d','D')
    return df
